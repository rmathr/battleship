...
function generateCoordinates(size) {
	...
	if (arguments.length === 1) {
		...
		randomArray[0] = Math.floor(Math.random() * 10)
		randomArray[1] = Math.floor(Math.random() * (10 - size))
...
function generateRandomCoordinates(size) {
	...
	function horizontalAxis() {
		randomArray[0] = Math.floor(Math.random() * 10)
		randomArray[1] = Math.floor(Math.random() * (10 - count))
		return executeHorizontalLoop(
			coordArray,
			randomArray[0],
			randomArray[1],
			count
	...
	function verticalAxis() {
		randomArray[0] = Math.floor(Math.random() * (10 - count))
		randomArray[1] = Math.floor(Math.random() * 10)
		return executeVerticalLoop(
			coordArray,
			randomArray[0],
			randomArray[1],
			count
...
class Player {
	...
	attackBoard(player, coordinates) {
		if (arguments.length > 1)
			return player.board.receiveAttack(coordinates, 'rightGameboard')
